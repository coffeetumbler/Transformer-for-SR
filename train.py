...
for epoch in range(args.num_epochs):
    ...
    with tqdm(len(train_size)) as t:
        ...
        for batch, origin, lr_img, lr_img_upscaled in enumerate(dataloader_train) :
            n_batch = origin[0]

            origin.to(device)
            lr_img.to(device)
            lr_img_upscaled.to(device)

            mid_result=model(lr_img, lr_img_upscaled)
            ...
            if batch %100 ==0:
                ...
                tqdm.write(f"loss: {loss:>6f}, [{current:5>d}/{train_size:>5d}]")
    ...
    loss_val=0
    with torch.no_grad():
        for batch, origin, lr_img, lr_img_upscaled in enumerate(dataloader_val):
            n_batch = origin.shape[0]
            ...
            mid_result = model(lr_img, lr_img_upscaled)
            loss_val = criterion(mid_result, origin)
